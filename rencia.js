...
const Rencia = new Client();
const config = require("./Config.json");

  function guvenli(kisiID) {
    let uye = Rencia.guilds.cache.get(config.guildID).members.cache.get(kisiID);
    let Renciasafe = config.whitelist || [];
    if (!uye || uye.id === Rencia.user.id || uye.id === config.OwnerID || uye.id === uye.guild.owner.id || Renciasafe.some(Rencia => uye.id === Rencia || uye.roles.cache.has(Rencia))) return true
  else return false};
  ...
  function cezalandir(kisiID, tur) {
    let userID = Rencia.guilds.cache.get(config.guildID).members.cache.get(kisiID);
    if (!userID) return;
    ...
    if (tur == "ban") return userID.ban({ reason: "Rencia Koruma Sistemi" }).catch()};

Rencia.on('ready', async () => {
  Rencia.user.setActivity(`${config.Status}`, { status: "idle"} ,{ type: 'PLAYİNG'})
  .then(console.log(`${Rencia.user.tag} İsmiyle Discord Bağlantısı kuruldu.`))
  .catch(() => console.log(`Bir hata ile karşılaştım.`))
});

//-                                                                        ROL KORUMA                                                                        -\\

Rencia.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Rencia Koruma Sistemi" });
  cezalandir(entry.executor.id, "jail");
  let user = Rencia.users.cache.get(entry.executor.id)
  let logKanali = Rencia.channels.cache.get(config.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setThumbnail(user.displayAvatarURL({dynamic:true})).setColor("BLUE").setTitle('Sunucuda izinsiz bir rol oluşturuldu!').setDescription(`${entry.executor} adlı yetkili tarafından sunucuda izinsiz bir rol oluşturuldu! \n\nSunucuda rolü oluşturan yetkilinin rolleri alındı ve cezalı rolü verildi!`).setFooter(`Rencia Koruma Sistemi`).setTimestamp())}});

Rencia.on("roleDelete", async role => {
